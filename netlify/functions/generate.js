...
exports.handler = async function (event) {
  ...
  try {
    ...
    creatorType = String(body.creatorType || "").slice(0, 40).trim().toLowerCase();
    if (Array.isArray(body.refine)) {
      refine = body.refine
        .map(function (r) { return String(r || "").slice(0, 60).trim(); })
  ...
  const refineLine = refine.length
    ? "Optional details the creator added: " + refine.join("; ") + ". Let these gently shape some of the ideas where they fit naturally. Do not force them into every handle."
    : "";

  const creatorLine = creatorType
    ? "The creator describes themselves as a " + creatorType + ". You may weave that identity into a few of the handles (for example a short tag like ugc, creates, or studio) where it reads naturally, but most handles should not include it. Never let it make a handle clunky."
    : "";
